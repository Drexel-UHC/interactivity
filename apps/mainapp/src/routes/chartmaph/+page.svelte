...
<script>
	...
	let statesToShow = feature(topojsonData, topojsonData.objects.states).features.map(d => d.properties.name); // show all states by default
	let zoomed = false;
	...
	function drawMap() {
		...
		svg.selectAll('.state')
			.data(feature(topojsonData, topojsonData.objects.states).features.filter(d => statesToShow.includes(d.properties.name)))
			.enter().append('path')
			.attr('class', 'state')
			.attr('d', path)
			.attr('stroke', 'navy')
	...
	function updateMap() {
		const selectedState = document.getElementById('states').value;
			...
			statesToShow = feature(topojsonData, topojsonData.objects.states).features.map(d => d.properties.name); 
			zoomed = false;
		} else {
			statesToShow = [selectedState];
			zoomed = true;
		}
		const svg = d3.select('#map');
		const projection = zoomed ? d3.geoIdentity().fitSize([width, height], feature(topojsonData, topojsonData.objects.states).features.find(d => d.properties.name === selectedState)) : d3.geoIdentity().fitSize([width, height], feature(topojsonData, topojsonData.objects.states));
		const path = d3.geoPath().projection(projection);
			svg.selectAll('.state')
			.data(feature(topojsonData, topojsonData.objects.states).features.filter(d => statesToShow.includes(d.properties.name)))
			.transition()
		...
		if (!zoomed) {
			svg.transition()
			.duration(750)
			.call(projection.animate({
				fit: feature(topojsonData, topojsonData.objects.states),
				padding: 20
			}));
		} else {
			const bounds = path.bounds(feature(topojsonData, topojsonData.objects.states).features.find(d => d.properties.name === selectedState));
...
<div>
	<select id="states" on:change={updateMap}>
